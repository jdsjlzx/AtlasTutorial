...
public class ActivityGroupDelegate {
  ...
  public void startChildActivity(ViewGroup container, String key, Intent intent) {
    ...
    if (contentActivity != null) {
      //已经添加过此Activity的View，直接复用
      container.addView(mLocalActivityManager.getActivity(key).
          getWindow().getDecorView(), new ViewGroup.LayoutParams(ViewGroup.LayoutParams.
  ...
  public void execStartChildActivityInternal(ViewGroup container, String key, Intent intent) {
    ...
    if (!TextUtils.isEmpty(bundleName)) {
      ...
      if (impl != null && impl.checkValidate()) {
        ...
        if (ActivityTaskMgr.getInstance().peekTopActivity() != null
  ...
  private void asyncStartActivity(final ViewGroup container, final String key,
      final String bundleName, final Intent intent) {
    final Activity current = ActivityTaskMgr.getInstance().peekTopActivity();
    final Dialog dialog =
        current != null ? RuntimeVariables.alertDialogUntilBundleProcessed(current, bundleName)
            : null;
    ...
    final BundleUtil.CancelableTask successTask = new BundleUtil.CancelableTask(new Runnable() {
      @Override public void run() {
        if (current == ActivityTaskMgr.getInstance().peekTopActivity()
            || currentActivitySize == ActivityTaskMgr.getInstance().sizeOfActivityStack() + 1) {
          performLaunchChildActivity(container, key, intent);
        }
